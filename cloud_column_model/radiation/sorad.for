c***********************       clirad-sw      ************************
cccshie 8/19/04
c     subroutine sorad (m,np,pl,ta,wa,oa,co2,
      subroutine sorad (m,np,jj2,pl,ta,wa,oa,co2,
...
c
c**********************************************************************
c------------     corrections for bugs      ----------------------------
c
c     a bug was found in "ntop=nctop(m)", it has been corrected to be
c     "ntop=nctop(i)".   dated march 28, 2000.
c
c 
c-----------------------------------------------------------------------
c 
...
c     parameter(nx1=nxf-4,ny1=nyf-4)   ! cccshie 8/19/04
      integer nx1, ny1  ! cccshie 9/15/04
       parameter (nx1=1,ny1=1)    ! cccshie 9/15/04
c-----input parameters
c     integer m,np,ict,icb
      integer m,np,ict,icb,jj2
...
!     real snt(m),cnt(m),x
      integer i,j,k,ntop
      real x
      integer,  allocatable :: nctop(:)
      real   ,  allocatable :: cwp(:,:,:)
      real   ,  allocatable :: dp(:,:)
      real   ,  allocatable :: wh(:,:)
      real   ,  allocatable :: oh(:,:)
      real   ,  allocatable :: scal(:,:)
      real   ,  allocatable :: swu(:,:)
      real   ,  allocatable :: swh(:,:)
      real   ,  allocatable :: so2(:,:)
      real   ,  allocatable :: df(:,:)
      real   ,  allocatable :: snt(:)
      real   ,  allocatable :: cnt(:)
c-----parameters for co2 transmission tables
c     integer nu,nw,nx,ny
      integer nu,nw,nx2,ny2 ! cccshie 9/15/04
c     parameter (nu=43,nw=37,nx=62,ny=101)
      parameter (nu=43,nw=37,nx2=62,ny2=101)
c     real w1,dw,u1,du,coa(nx,ny),cah(nu,nw)
...
cccshie 9/18/04
c     call soluv (m,np,wh,oh,dp,overcast,cldwater,
      call soluv (m,np,jj2,wh,oh,dp,overcast,cldwater,
     *            cwp,taucld,reff,ict,icb,fcld,cosz,
     *            taual,ssaal,asyal,rsuvbm,rsuvdf,
     *            flx,flc,fdiruv,fdifuv,fdirpar,fdifpar)
c-----compute and update solar ir fluxes
c     call solir (m,np,wh,dp,overcast,cldwater,
      call solir (m,np,jj2,wh,dp,overcast,cldwater,
     *            cwp,taucld,reff,ict,icb,fcld,cosz,
...
c     eq. (3.19)
c       call rflx(m,np,swu,u1,du,nx,swh,w1,dw,ny,coa,df)
        call rflx(m,np,swu,u1,du,nx2,swh,w1,dw,ny2,coa,df)
...
cccshie 8/19/04
         if(k.eq.1)then
           rflux(i,jj2,3)=rflux(i,jj2,3)-df(i,k)  ! downward SW TOA
         endif
        enddo
...
cccshie 8/9/04
         if(k.eq.(np+1))then
           rflux(i,jj2,1)=rflux(i,jj2,1)-df(i,k)  ! downward SW surface ! for 3D
         endif
         enddo
        endif
       enddo
